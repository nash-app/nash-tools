...
PRIORITY_FEE = "200000"
WSOL_OUTPUT_MINT = "So11111111111111111111111111111111111111112"
SOLANA_RPC_URL = "https://api.mainnet-beta.solana.com"
MAX_RETRY_COUNT = 5

DESCRIPTION = (
    "Sells a token for SOL on Raydium. Takes token address, token amount, "
    "and slippage in basis points."
)
...
class LLMParams(BaseModel):
    ...
    amount: Decimal = Field(..., description="Amount of token to sell", gt=0)
    slippage_bps: int = Field(
        ..., description="Slippage in basis points", gt=0, le=1000
    )

    @field_validator("token_address")
    def validate_token_address(cls, v: str) -> str:
        if v == WSOL_OUTPUT_MINT:
            raise ValueError("Cannot sell SOL for SOL")
        return v
...
def get_swap_computation(
    ...
    try:
        response = requests.get(
            f"{RAYDIUM_COMPUTE_SWAP_IN_API_ENDPOINT}?inputMint={token_address}&"
            f"outputMint={WSOL_OUTPUT_MINT}&amount={token_amount}&"
            f"slippageBps={slippage_bps}&txVersion=V0&fixed=in"
        )
...
def get_swap_transaction(
    ...
    try:
        ...
        trade_data = {
            ...
            "wallet": str(wallet_pubkey.pubkey()),
            "inputAccount": input_token_account,
        }
...
def process_transaction(transaction_data: dict, solana_keypair: Keypair) -> str:
    ...
    try:
        ...
        if "insufficient lamports" in error_msg:
            raise RaydiumError(
                "Insufficient SOL to cover transaction fees. Need at least 0.004 SOL for fees."
            )
...
def format_error_message(error_type: str, details: str) -> str:
    return f"raydium_swap_sell_tool error: {error_type} - {details}"


def tool_function(token_address: str, amount: Decimal, slippage_bps: int) -> str:
    """Sells a token for SOL on Raydium"""
    try:
        # Validate parameters
        try:
        ...
        # Core swap logic
        token_amount = int(params.amount)  # No decimal conversion needed for tokens
            ...
            for _ in range(5):
                try:
                    ...
                    if tx_hash:
                        try:
                            send_notification(
                                f"Sold {amount} {token_address} for SOL on Raydium"
                            )
                        except RaydiumError:
...
if __name__ == "__main__":
    ...
    output = tool_function(
        ...
        amount=Decimal(
            "3592000"
        ),  # Amount in base units (no decimal conversion needed)
        slippage_bps=1000,
    )
    print(output)
